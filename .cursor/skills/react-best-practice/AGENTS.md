...
> generating, or refactoring client-side React codebases at Vercel. Humans  
...
Comprehensive performance optimization guide for client-side React applications, designed for AI agents and LLMs. Contains 40+ rules across 7 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.
...
Keep the import behind the feature flag so the extra code only loads when the feature is activated.
...
export default function AppRoot({ children }: { children: React.ReactNode }) {
  useEffect(() => {
    void import("@/libs/analytics").then((mod) => {
      mod.initAnalytics();
    });
  }, []);
...
Preload only when the feature is enabled to avoid unnecessary downloads.
